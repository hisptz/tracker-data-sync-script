import {ConflictSummary, ImportSummary, ImportSummaryResponse, Summary} from "./interfaces";
...
export default class SummaryService {
    ...
    static async updateUploadSummary(page: number, importSummary: ImportSummary) {
        const {response} = importSummary ?? {}

        if (response) {
            const summaryData: Summary = await this.getAllSummaryData();
            if (summaryData) {
                const {upload: {updated, deleted, ignored, imported}} = summaryData ?? {};
                const updatedSummary = {
                    ...
                    upload: {
                        ...
                        conflicts: this.getConflictsFromImportSummaryResponse(page, response)
                    }
                }
    ...
    static async updateDownloadSummary(page: number, status: "success" | "error" | "timeout") {
        if (page && status) {
            const summaryData: Summary = await this.getAllSummaryData();
            if (summaryData) {

                let {download: {downloaded, errors, timedOut, errorPages, timedOutPages}} = summaryData ?? {};
        ...
        try {
            ...
            if (summaryData) {
                ...
                if (endTime && startTime) {
                    const duration = Duration.fromMillis(endTime - startTime).shiftTo("hours", "minutes", "seconds")
                    return {
    ...
    private static getConflictsFromImportSummaryResponse(page: number, response: ImportSummaryResponse): ConflictSummary[] {
        if (response) {
            const conflicts = response.importSummaries;
            return compact(flattenDeep(conflicts.map(({reference, conflicts}) => {
